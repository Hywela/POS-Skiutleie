...
function send_rec_form(form) {
	...
	//$('#nameForm').submit();
	 ...
	 var receiptID = parseInt($('#receipt_id_updater').text());
if(receiptID > 0){	
	...
	var price = parseFloat($("#total_price").val()) +  parseFloat($("#old_sum").text()) ;
	//alert(price);
	if( payment_method == 1){
		payd_card = parseFloat($("#total_price").val());
	}
	...
	if(itemList.length > 0){
		...
		//$('#overview').focus();
		updateComment( form.comment.value, receiptID );
	
	$.ajax({
		url: "php_script/update_receipt.php",
		type: "post",
		data: {
			'receiptID':receiptID,
			'price':price,
